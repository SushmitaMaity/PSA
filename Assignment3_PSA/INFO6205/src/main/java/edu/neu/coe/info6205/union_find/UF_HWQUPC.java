...
public class UF_HWQUPC implements UF {
    ...
    public void countPairs(int n) {
        int numPairs = 0;
        UF_HWQUPC ufCon = new UF_HWQUPC(n, true);
        Random r = new Random();
        for (int i = 0; i < n; i++) {
            while (count != 1) {
                numPairs++;
                int x = r.nextInt(n);
                int y = r.nextInt(n);
                if (!ufCon.connected(x, y)) {
                    ufCon.union(x, y);
                    count--;
                }
            }
        }
        System.out.println("Number of pairs generated: " + numPairs);
    }


    public static void main(String[] args) {
        int[] n = {10, 50, 100, 350, 500, 2000, 4000, 10000, 50000};
        int len = n.length;
        for (int i = 0; i < len; i++) {
            UF_HWQUPC uf = new UF_HWQUPC(n[i]);
            uf.countPairs(n[i]);
            System.out.println("Number of objects: " + n[i] + " Connected components: " + uf.components());
        }
    }
}
